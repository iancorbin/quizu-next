      ...
      <div className="relative overflow-hidden" style={{ background: "var(--grad-game)" }}>
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="mono text-[10px] font-bold px-2 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.06)", color: "var(--cyan)" }}>+10 pts</span>
            {takenNum > 0 && <span className="mono text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>{takenNum.toLocaleString()} played</span>}
            {quiz.category && (
              <Link href={`/topic/${quiz.category}`} className="mono text-[11px] capitalize hover:text-white transition-colors" style={{ color: "rgba(255,255,255,0.3)" }}>{quiz.category}</Link>
            )}
          </div>
          <h1 className="display text-2xl sm:text-3xl font-extrabold text-white leading-tight tracking-tight mt-3 max-w-3xl">
            {quiz.title}
          </h1>
          {quiz.description && (
            <p className="mt-2 text-sm leading-relaxed max-w-xl" style={{ color: "rgba(255,255,255,0.35)" }}>{quiz.description}</p>
          )}
        </div>
      </div>
